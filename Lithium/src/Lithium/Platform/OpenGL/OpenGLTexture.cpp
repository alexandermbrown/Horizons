...
namespace li
{
		...
		: m_Width(width), m_Height(height)
	{
		...
		glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, ConvertOpenGL::FilterType(minFilter));
		glTextureParameteri(m_RendererID, GL_TEXTURE_MAG_FILTER, ConvertOpenGL::FilterType(magFilter));

		glTextureParameteri(m_RendererID, GL_TEXTURE_WRAP_S, ConvertOpenGL::WrapType(wrapS));
		glTextureParameteri(m_RendererID, GL_TEXTURE_WRAP_T, ConvertOpenGL::WrapType(wrapT));
	}
	...
	OpenGLTexture2D::OpenGLTexture2D(size_t imageSize, uint8_t* rawData, WrapType wrapS, WrapType wrapT, FilterType minFilter, FilterType magFilter)
	{
		...
		stbi_uc* data = nullptr;
		data = stbi_load_from_memory(rawData, imageSize, &width, &height, &channels, 0);
	...
	{
		glFramebufferTexture2D(ConvertOpenGL::FramebufferTarget(target), ConvertOpenGL::FramebufferAttachment(attachment), GL_TEXTURE_2D, m_RendererID, 0);
	}
